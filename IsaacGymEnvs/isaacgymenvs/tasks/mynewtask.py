...
def normalize_color(counter,rgba):
    # scale to 0, 1 reshape(-1,196608)
    nml_rgb = rgba[..., :3].to(torch.float32) / 255
    """ nml_rgb = rgba[..., :3].to(torch.float32)
    
    nml_rgb = nml_rgb.permute((2, 0, 1))
    pic = toPIL(nml_rgb)
    pic.save("/home/roland/MyNewTask/rgb%d.png"%(counter))
    print('Saved the img from the  camera..................') """
    nml_rgb = nml_rgb.reshape(-1,270000)
    return nml_rgb
...
def matrix_to_euler_angles(matrix, convention: str):
    ...
    o = (
        _angle_from_tan(
            convention[0], convention[1], matrix[..., 2], False, True
        ),
        central_angle,
        _angle_from_tan(
            convention[2], convention[1], matrix[..., 0, :], True, True
        ),
    )
...
class MyNewTask(VecTask):
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        # dimensions
        # obs include: cubeA_pose (7) + cubeB_pos (3) + eef_pose (7) + q_gripper (2) img size 256*256*3 196608 196628
        self.cfg["env"]["numObservations"] = 270009 if self.control_type == "osc" else 26
        ...
        # [0, 0.1963, 0, -2.6180, 0, 2.9416]
        # -1.3958e+00, -1.7708e+00, -1.7120e+00, -1.2278e+00,  1.5816e+00, 1.3618e+00
        self.franka_default_dof_pos = to_torch(
            [-1.3959e+00, -1.7157e+00, -1.4076e+00, -1.5868e+00,  1.5845e+00, 1.3615e+00, 0.04, 0.04], device=self.device
        )
        # 2.049, 3.1416, 2.6878
        self.fingers_open_state =  to_torch([0.04, 0.04], device=self.device)
        self.fingers_close_state =  to_torch([0, 0, 0], device=self.device)
        ...
        self.AonB = torch.zeros(self.num_envs,1)
        self.AonBs = torch.zeros(self.num_envs,5)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        franka_asset_file = "urdf/ur_description/urdf/ur10_robot.urdf"
        ...
        franka_dof_damping = to_torch([0, 0, 0, 0, 0, 0, 100, 100], dtype=torch.float, device=self.device)
        ...
        local_transform.p = gymapi.Vec3(-0.2403, -0.0087, 0.04259)
        local_transform.r = gymapi.Quat(-0.57024245, -0.41811906,  0.41811906,  0.57024245)
        ...
        table_stand_pos = [0, -0.8, 0]
        table_stand_opts = gymapi.AssetOptions()
        table_stand_opts.fix_base_link = True
        table_stand_asset = self.gym.create_box(self.sim, *[0.2, 0.2, 2.0], table_opts)
        ...
        franka_start_pose.p = gymapi.Vec3(0, -0.8, 1.0)
        ...
        for i in range(self.num_envs):
            ...
            attached_body = self.gym.get_actor_rigid_body_handle(env_ptr, franka_actor, 7)
            #attached_body = self.gym.find_actor_rigid_body_handle(env_ptr, franka_actor, 5)
            


            self.gym.attach_camera_to_body(camera_handle, env_ptr, attached_body, local_transform, gymapi.FOLLOW_TRANSFORM)
            self._camera_handle.append(camera_handle)
        ...
        self.color_buf = torch.zeros(
                    self.num_envs, camera_props.height * camera_props.width *3,
                    dtype=torch.float,
                    device=self.device)

        self.depth_buf = torch.zeros(
                    self.num_envs, camera_props.height * camera_props.width,
        ...
        # get the finger tips handle for calculate the reward
    ...
    def init_data(self):
        ...
        # make sure all the camera data be ready for retrieval
        # Get total DOFs
        ...
        self.dof_vel = torch.zeros_like(self._qd)
        ...
        hand_joint_index = self.gym.get_actor_joint_dict(env_ptr, franka_handle)['arm_gripper_joint']
    ...
    def _update_states(self):
        self.states.update({
            ...
            "color_img": self.color_buf,
            "depth_img": self.depth_buf,
            #"eef_1f_pos": self._eef_1f_state[:, :3],
            #"eef_2f_pos": self._eef_2f_state[:, :3],
            #"eef_3f_pos": self._eef_3f_state[:, :3],
            # Cubes
    ...
    def refresh_image_tensors(self):
        ...
        for i in range(self.num_envs):
            ...
            self.color_buf[i, ...] = normalize_color(self.counter,color_tensor) 
            self.counter = self.counter +1
            #self.color_buf[i, ...] = color_tensor
            depth_tensor = gymtorch.wrap_tensor(self.gym.get_camera_image_gpu_tensor(
                    self.sim, env_handle, camera_handle, gymapi.IMAGE_DEPTH))
            depth_tensor = depth_tensor.view(-1,90000)
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.reset_buf[:] = compute_franka_reward(
            self.reset_buf, self.progress_buf, self.actions, self.states, self.reward_settings, self.max_episode_length,self.rv[:,2]
        )

    def compute_observations(self):
        self._refresh()
        #obs = ["cubeA_quat", "cubeA_pos", "cubeA_to_cubeB_pos", "eef_pos", "eef_quat","q_gripper"]
        obs = ["color_img","eef_pos", "eef_quat","q_gripper"]
        #obs += ["q_gripper"] if self.control_type == "osc" else ["q"]
        self.obs_buf = torch.cat([self.states[ob] for ob in obs], 
                        dim=-1)
    ...
    def reset_idx(self, env_ids):
        ...
        # This method only used after each epoch, reset the arm and objects
        ...
        pos = tensor_clamp(
            ...
            self.franka_dof_lower_limits.unsqueeze(0), self.franka_dof_upper_limits.unsqueeze(0))

        # Overwrite gripper init pos (no noise since these are always position controlled)
        pos[:, -3:] = self.franka_default_dof_pos[-3:]
        #print('pos~~~~~~~~~',pos)
...
# ~~~~~~~~~~~~~
    ...
    #    - e.g. apply actions
    # u_arm are the 6 degree movement in cartesian system
        ...
        #print('self._effort_control:',self._effort_control[0])
    ...
    def post_physics_step(self):
        ...
        # need to check the env_ids for each env, if reset_buf[env_ids] = 1, then reset this env.
        if len(env_ids) > 0:
            self.reset_idx(env_ids)
        Rq = self.states["eef_quat"].contiguous()
        self.rv = tgm.quaternion_to_angle_axis(Rq)
...
def compute_franka_reward(
    reset_buf, progress_buf, actions, states, reward_settings, max_episode_length,rv
):
    ...
    # the joint angle change
    
    rv = rv.unsqueeze(1)
    dr = torch.norm(rv, dim=1)
    #dr = dr/2.0
    #print(dr[0])
    # distance from hand to the cubeA
    d = torch.norm(states["cubeA_pos_relative"], dim=-1)
    dfss =  torch.norm(states["eef_lf_pos"] - states["eef_rf_pos"], dim=-1)
    dfs = (cubeA_size * 1.2 - dfss) 
    dfs = torch.clamp(dfs,0,cubeA_size[0]* 1.2)
    #d_lf = torch.norm(states["cubeA_pos"] - states["eef_lf_pos"], dim=-1)
    #d_rf = torch.norm(states["cubeA_pos"] - states["eef_rf_pos"], dim=-1)
    d_lf = torch.norm(states["cubeA_pos"] - states["eef_lf_pos"], dim=-1)
    d_rf = torch.norm(states["cubeA_pos"] - states["eef_rf_pos"], dim=-1)
    dist_reward = 1 - torch.tanh(0.01 + 10.0 * (dr +3*d + d_lf + d_rf) / 7.0)
    #dist_reward = 1 - torch.tanh(10.0 * ( dr + 3*d +  2*dfs +  d_lf + d_rf ) / 8.0)
    # reward for c cube
    cubeA_height = states["cubeA_pos"][:, 2] - reward_settings["table_height"]
     
    #cubeA_lifted = (cubeA_height - cubeA_size* 0.7) / cubeB_size[0]
    cubeA_lifted = (cubeA_height - cubeA_size* 0.5) / cubeB_size[0]
    cubeA_lifted = torch.clamp(cubeA_lifted,0,0.7)
    ...
    drr = dr/7.0
    align_reward = (1 - torch.tanh(10.0 * (d_ab+drr))) * (cubeA_lifted > 0.5)
    ...
    cubeA_on_cubeB = torch.abs(cubeA_height - target_height - cubeB_size/2.0) < 0.01
    ...
    #cubeA_on_cubeB = ((cubeA_height - target_height - cubeB_size/2.0 ) > -0.005) & ((cubeA_height - target_height - cubeB_size/2.0 ) < 0.005)
    cubeA_place_on_cubeB = cubeA_align_cubeB & cubeA_on_cubeB
    gripper_away_from_cubeA = (d + dfss - cubeA_size) /0.3
    #gripper_away_from_cubeA = (d-0.01) / 0.09
    gripper_away_from_cubeA = torch.clamp(gripper_away_from_cubeA,0,1)
    
    #away_reward = cubeA_place_on_cubeB *(1 - torch.tanh(1.0/(d + dfss - cubeA_size))) 

    stack_reward = cubeA_place_on_cubeB * gripper_away_from_cubeA
    #dist_reward = torch.max(dist_reward,stack_reward)
    #align_reward = torch.max(align_reward,cubeA_place_on_cubeB)
    # Compose rewards

    # We either provide the stack reward or the align + dist reward
  
    rewards = reward_settings["r_dist_scale"] * dist_reward + reward_settings["r_lift_scale"] * lift_reward + reward_settings[
           "r_align_scale"] * align_reward + reward_settings["r_stack_scale"] * stack_reward
    #rewards = reward_settings["r_dist_scale"] * dist_reward + reward_settings["r_lift_scale"] * lift_reward 
    # Compute resets
    #cubeA_place_is_on_cubeB = torch.where(cubeA_place_on_cubeB==1, torch.ones_like(cubeA_place_on_cubeB), torch.zeros_like(cubeA_place_on_cubeB))
    """ rewards = torch.where(
        stack_reward > 0.8,
    ...
    reset_buf = torch.where((progress_buf >= max_episode_length - 1)  | (stack_reward > 0.5), torch.ones_like(reset_buf), reset_buf)
